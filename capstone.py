...
st.markdown("")
st.markdown("For this project, a historical reading from US Embassy Jakarta's air quality meters located in Central and South Jakarta will be used. The historical data contains daily AQI measurement from January 2016 to the October 11th 2022.")
...
with c1:
    ...
    styler_yearly = data_yearly[data_yearly["Loc"] == "Central Jakarta"][["Year", "AQI min", "AQI mean", "AQI max"]].style.hide_index().format(precision = 2).bar(subset = ["AQI min", "AQI mean", "AQI max"])
...
with c2:
    ...
    styler_yearly = data_yearly[data_yearly["Loc"] == "South Jakarta"][["Year", "AQI min", "AQI mean", "AQI max"]].style.hide_index().format(precision = 2).bar(subset = ["AQI min", "AQI mean", "AQI max"])
    st.write(styler_yearly.to_html(), unsafe_allow_html = True)

st.markdown("")
st.markdown("Looking at the yearly summary from the historical data, it is clearly visible that the AQI value was the highest during 2016. There was a significant starting in 2017. The values dropped even more during the COVID lockdown in 2020 to 2021. However, we see an increase again in 2022 after the COVID lockdown was relaxed. The maximum value in 2022 is almost the same as it was before the COVID lockdown, with South Jakarta area seeing a very drastic increase")
